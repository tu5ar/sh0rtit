...
// src/util/projects/require-project-context.ts
async function requireProjectContext(client, command, projectName) {
...
// src/util/command-arguments.ts
function parseSubcommandArguments(argv, command) {
  const flagsSpecification = getFlagsSpecification(command.options);
  return parseArguments(argv, flagsSpecification);
}
function validateRequiredArguments(args, required) {
...
`
  ...
  if (!hadExistingStagingVersion && !opts.skipPrompts) {
    ...
    output_manager_default.warn(
      `There are other staged changes. Review with ${import_chalk.default.cyan(withGlobalFlags2(client, "routes list --diff"))} before promoting.`
    );
  }
}
...
async function resolveRoutes(client, routes, identifiers) {
  ...
  for (const identifier of identifiers) {
    ...
    if (!route) {
      ...
      output_manager_default.error(
        `No route found matching "${identifier}". Run ${import_chalk.default.cyan(withGlobalFlags2(client, "routes list"))} to see all routes.`
      );
      return null;
    }
  ...
  if (matchingVersions.length === 0) {
    return {
      error: `Version "${identifier}" not found. Run ${import_chalk.default.cyan(
        withGlobalFlags2(client, "routes list-versions")
...
export {
  ...
  getSrcSyntaxLabel,
  isTargetTransform,
  withGlobalFlags2 as withGlobalFlags,
