...
import {
  progress
} from "./chunk-I2XE3GMB.js";
import {
  isJSONObject,
  printIndications,
  require_dist as require_dist2,
  sleep
} from "./chunk-GKLT2ZGC.js";
import {
  suggestNextCommands
} from "./chunk-XBN2O34P.js";
import {
  CommandTimeout
} from "./chunk-7RTO3VPE.js";
import {
  showPluginTipIfNeeded
} from "./chunk-HT2XWSAJ.js";
import {
  stamp_default
...
async function printEvents(client, urlOrDeploymentId, { mode, onEvent, onAliasAssigned, quiet, findOpts }, abortController) {
...
function displayBuildLogs(client, deployment, follow = true, onAliasAssigned) {
...
function collectBuildLogs(client, deployment, follow = true) {
  const logs = [];
  const abortController = new AbortController();
  const promise = events_default(
    client,
...
async function processDeployment({
  ...
  const onAliasAssigned = (event) => {
    aliasAssignedController.abort(event);
  };
  const clientOptions = {
    teamId: now.currentTeam ?? void 0,
    apiUrl: now._apiUrl,
    token,
    ...
    for await (const event of (0, import_client.createDeployment)(clientOptions, requestBody)) {
      ...
      if (event.type === "created") {
        ...
        if (isProdDeployment && !requestBody.target) {
          indications.push({
            type: "notice",
            payload: "This is your project\u2019s first deployment, so it was assigned to production. Future deployments will be preview deployments unless you use --prod.",
        ...
        if (noWait) {
          deployment.indications = indications;
          return deployment;
        }
