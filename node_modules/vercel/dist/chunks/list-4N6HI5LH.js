...
import "./chunk-UMA66MKW.js";
import "./chunk-KT4XXKJK.js";
import "./chunk-LXRK7KPA.js";
...
function getGroupResolvedAt(group) {
  const resolvedTimes = (group.alerts ?? []).map(
    (alert) => normalizeTimestamp(alert.recordedResolvedAt ?? alert.resolvedAt)
...
function getStatus(group) {
  ...
  if (normalizedStatus === "resolved") {
    ...
    if (startedAt && resolvedAt && resolvedAt >= startedAt) {
      return `resolved after ${(0, import_ms.default)(resolvedAt - startedAt)}`;
    }
    return "resolved";
  }
  return group.status || "-";
...
function printGroups(groups) {
  ...
  const rows = [
    ...
    ...groups.map((group) => {
      const row = [
        import_chalk.default.bold(truncateEnd(getGroupTitle(group), 80)),
