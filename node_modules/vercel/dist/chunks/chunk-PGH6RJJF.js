...
async function explode(paths, { accepts }) {
  ...
  const list = async (file) => {
    ...
    if (s.isDirectory()) {
      let all;
      try {
        all = await import_fs_extra.default.readdir(file);
      } catch (err) {
        const code2 = err instanceof Error && "code" in err ? err.code : void 0;
...
async function installBuilders(buildersDir, buildersToAdd, span, installReasons) {
  if (!span) {
...
function pinBuilderSpecs(specs) {
  ...
  for (const spec of specs) {
    ...
    if (parsed.name && isBareSpec(parsed)) {
      ...
      if (version2) {
        pinnedSpecs.set(spec, `${parsed.name}@${version2}`);
      }
    }
  }
  return pinnedSpecs;
...
async function resolveBuilders(buildersDir, builderSpecs, resolvedSpecs) {
  ...
  for (const spec of builderSpecs) {
    ...
    try {
      ...
      if (isBareSpec(parsed) && peerVersion && pkgPath.startsWith(buildersDir) && !(0, import_semver2.satisfies)(builderPkg.version, peerVersion)) {
...
function isContainerImage(value) {
  return typeof value === "object" && value !== null && "runtime" in value && value.runtime === "container";
}
function isLambda(v) {
...
async function resolveFunctionConfiguration(src, vercelConfig, service) {
...
async function writeBuildResultV2(args) {
  ...
  const overrides = {};
  const functionConfiguration = build2.src ? await resolveFunctionConfiguration(build2.src, vercelConfig, service) : {};
...
async function writeBuildResultV3(args) {
  ...
  if (isContainerImage(output)) {
    ...
    await writeContainerImage(outputDir, output, path2, functionConfiguration);
  } else if (isLambda(output)) {
    injectServiceEnvVars(
      output,
      service && isExperimentalService(service) ? service : void 0,
...
async function writeContainerImage(outputDir, containerImage, path2, functionConfiguration) {
  ...
  const architecture = functionConfiguration?.architecture ?? containerImage.architecture;
  const memory = functionConfiguration?.memory ?? containerImage.memory;
  const maxDuration = functionConfiguration?.maxDuration ?? containerImage.maxDuration;
  const maxConcurrency = functionConfiguration?.maxConcurrency ?? containerImage.maxConcurrency;
  const regions = functionConfiguration?.regions ?? containerImage.regions;
  const functionFailoverRegions = functionConfiguration?.functionFailoverRegions ?? containerImage.functionFailoverRegions;
  const experimentalTriggers = functionConfiguration?.experimentalTriggers ?? containerImage.experimentalTriggers;
  const supportsCancellation = functionConfiguration?.supportsCancellation ?? containerImage.supportsCancellation;
  ...
  await import_fs_extra5.default.writeJSON(
    join4(dest, ".vc-config.json"),
    {
      ...
      ...functionFailoverRegions !== void 0 ? { functionFailoverRegions } : {},
      ...experimentalTriggers !== void 0 ? { experimentalTriggers } : {},
      ...supportsCancellation !== void 0 ? { supportsCancellation } : {}
    },
    { spaces: 2 }
  );
}
...
async function writeLambda(repoRootPath, outputDir, lambda, path2, functionConfiguration, existingFunctions, standalone = false) {
  ...
  const maxConcurrency = functionConfiguration?.maxConcurrency ?? lambda.maxConcurrency;
...
function filesWithoutFsRefs(files, repoRootPath, standalone) {
  const symlinkDirs = standalone ? Object.entries(files).filter(([, f]) => f.type === "FileFsRef" && isSymbolicLink(f.mode)).map(([p]) => `${normalizePath(p)}/`) : [];
  let filePathMap;
  const out = {};
